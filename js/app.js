...
async function approveSongRequestWithTitle(requestId, title, level, isHot) {
  const cleanTitle = String(title || '').trim();
  if (!cleanTitle) throw new Error('承認する曲名を入力してください。');

  const { data, error } = await supabase.rpc('approve_song_request_with_title', {
...
import { listUserSummaries, getUserSkillTargets, getSongRateComparison, getSongPersonalBestHistory, getSongOptionDistribution, getMyFavorites, addFavorite, removeFavorite } from './users.js?v=21_160';
...
document.addEventListener('click', async e => {
  ...
  if (adminApproveRequest) {
    try {
      ...
      const titleChanged = req && title !== req.title;
      const confirmMessage = titleChanged
        ? `曲名を「${req.title}」から「${title}」へ修正して、OTHERとして承認しますか？`
        : 'この登録依頼をOTHERとして承認しますか？';

      if (!await showSiteConfirm(confirmMessage, '登録依頼の承認', '承認する')) return;

      await approveSongRequestWithTitle(requestId, title, level, false);
  ...
  if (adminHotRequest) {
    try {
      ...
      if (!await showSiteConfirm(confirmMessage, 'HOT承認', '承認する')) return;

      await approveSongRequestWithTitle(requestId, title, level, true);
      await loadAdminRequests();
    } catch (e) {
